...
function validarPassword(password) {
  ...
  if (!hasLowerCase.test(password)) return "La contraseña debe contener al menos una letra minúscula.";
  return null;
}
...
export async function registrarUsuarioPublico(req, res) {
  try {
    ...
    if (nombre.length > 50) {
       return res.status(400).json({ error: "El nombre no puede exceder los 50 caracteres." });
    }
...
export async function actualizarPerfil(req, res) {
  try {
    ...
    if (password) {
      ...
      const match = await bcrypt.compare(passwordActual, currentUser.password_hash);
      if (!match) {
