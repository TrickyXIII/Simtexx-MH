...
export default function MiPerfil() {
  ...
  useEffect(() => {
    // Cargar datos del localStorage para rellenar el formulario
    const storedUser = JSON.parse(localStorage.getItem("usuarioActual") || "{}");
    setUsuario({
      nombre: storedUser.nombre || "",
      correo: storedUser.correo || "",
      rol: storedUser.rol || storedUser.rol_nombre || "Usuario"
    });
  }, []);
    ...
    <>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: '40px', minHeight: 'calc(100vh - 140px)' }}>
        ...
        <form
          ...
          style={{
            display: "flex", flexDirection: "column", width: "350px", gap: "15px",
            background: "white", padding: "30px", borderRadius: "10px", boxShadow: "0 4px 15px rgba(0,0,0,0.1)"
          }}
        >
          <div style={{textAlign: 'center', marginBottom: '10px', color: '#666'}}>
            Rol actual: <strong>{usuario.rol}</strong> <br/>
            <small>(No es posible editar tu propio rol)</small>
          </div>
          ...
          <label style={{fontWeight: 'bold'}}>Confirmar Contraseña</label>
          <input
            type="password"
          ...
          <button 
            type="submit" 
            style={{ marginTop: "15px", padding: "12px", background: "#2c3e50", color: "white", border: "none", borderRadius: "5px", cursor: "pointer", fontWeight: "bold", fontSize: "16px" }}
          >
            Actualizar Datos
          </button>
          ...
          <button
            ...
            style={{ padding: "10px", background: "#95a5a6", color: "white", border: "none", borderRadius: "5px", cursor: "pointer" }}
          >
            Cancelar
          </button>
        </form>
